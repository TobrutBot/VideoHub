    ...
    <div className="relative min-h-screen bg-gray-900">
      ...
      <main className="relative container mx-auto px-4 py-8">
        <div className="max-w-[1200px] mx-auto">
          {videos.length === 0 ? (
            <div className="flex items-center justify-center h-[200px] bg-gray-800 bg-opacity-75 text-white">
              <p>Tidak ada video yang tersedia. Silakan tambahkan video melalui bot Telegram.</p>
            </div>
          ) : (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                      {slideVideos.map((video, index) => {
                        ...
                        return (
                          ...
                          >
                            {/* Thumbnail sebagai fallback */}
                            <img
                              src={video.thumbnailUrl}
                              alt={`Thumbnail video ${globalIndex + 1}`}
                              className="absolute inset-0 w-full h-full object-cover"
                              style={{ display: isLoading[globalIndex] || videoErrors[globalIndex] ? 'block' : 'none' }}
                            />
